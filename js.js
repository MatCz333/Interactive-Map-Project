...
function NavigationBarAnimatePort() {
    ...
    $("#side-panel").css("background-image","url('Photos/Frames/sidepanelframetest.png')");
     $("#side-panel").css("background-size","100% 100%");
...
function HideShips(shipDetails, shipsArray, iterator) {
    ...
    if (filterShips(shipDetails[iterator][3], 1800, 1820) <= 1820)
        shipsArray.setVisible(false);
}
...
function moveShipAlongPath(ship, routeNumber) {
    ...
    var threadNumber = setInterval(function() {
        ...
        if ((ship.getPosition().lat != ship.getAnimationPosition().lat) &&
            (ship.getPosition().lng != ship.getAnimationPosition().lng)) {
    ...
    success: function(data) {
        ...
        function setContentWindowSchema(number, shipData) {
            var contentString = '<div id="content">' +
                '<div id="siteNotice">' +
                '</div>' +
                '<h2 id="firstHeading" class="firstHeading" style="font-family:\'Niconne\',cursive;">' + shipDetails[number][0] + '</h2>' +
                '<div id="bodyContent" style="font-family:\'Merriweather\',\'Georgia\';">' +
                '<p> Number of convicts onboard: ' + numberofConvicts[number] + '</p>' +
                '<p> Names of convicts onboard: <br/> ' + namesofConvicts[number] + '</p>' +
                '<p> Date of Arrival: ' + shipDetails[number][3] + '</p>' +
                '<p> Port of Arrival: ' + shipDetails[number][1] + '</p>' +
                '</div>' +
                '</div>';
        ...
        $("#listOfConvicts").click(function() {
            ...
            var cloneContainer =  $( ".convictContainer" );
             ...
             for ( var x=0; x<numberofConvicts; x++){
                //set name
                 $(cloneContainer).find("figcaption").text(namesofConvicts[Math.floor((Math.random() * namesofConvicts.length) + 0)].replace("< br/>",""));
               
                //set photo
                  $(cloneContainer).find(".myImage").attr("src","Photos/Convicts/Convict" + Math.floor((Math.random() * 8) + 1) + ".jpg");
            
                  // set text
                   $(cloneContainer).find(".myheading").text(convictStories[Math.floor((Math.random() * convictStories.length) + 0)])
